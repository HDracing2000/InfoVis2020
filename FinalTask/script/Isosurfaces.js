function Isosurfaces( volume, isovalue )
{
    ...
    // Define material Color from color map
    var color = new THREE.Color().setHex( cmap[ isovalue ][1] );
    var color4 = new THREE.Vector4(color.r, color.g, color.b, 1.0 );    
        ...
        for ( var y = 0; y < volume.resolution.y - 1; y++ )
        {
            for ( var x = 0; x < volume.resolution.x - 1; x++ )
            {
                ...
                for ( var j = 0; lut.edgeID[index][j] != -1; j += 3 )
                {
                    ...
                    var v01 = interpolated_vertex( v0, v1, isovalue, vid0, vid1, indices, cell_index-1 );
                    var v23 = interpolated_vertex( v2, v3, isovalue, vid2, vid3, indices, cell_index-1 );
                    var v45 = interpolated_vertex( v4, v5, isovalue, vid4, vid5, indices, cell_index-1 );
    ...
    function interpolated_vertex( v0, v1, s , vid0, vid1, indices, cell_index )
    {
        var lines = volume.resolution.x;
        var slices = volume.resolution.x * volume.resolution.y;
        var id0 = cell_index + vid0[0] * 1 + vid0[1] * lines + vid0[2] * slices;
        var id1 = cell_index + vid1[0] * 1 + vid1[1] * lines + vid1[2] * slices;
        var s0 = volume.values[ id0 ][0];
        var s1 = volume.values[ id1 ][0];
        var ds0 = Math.abs( s0 - s );
        var ds1 = Math.abs( s1 - s )
        
        return new THREE.Vector3().addVectors( v0.multiplyScalar(ds1), v1.multiplyScalar(ds0) ).divideScalar( ds0 + ds1 );
